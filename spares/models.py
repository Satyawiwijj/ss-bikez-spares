...
class SparesItem(models.Model):
    ...
    category = models.ForeignKey(
        SparesCategory, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class ItemRackBin(models.Model):
    """Which racks/bins an item is stored in — from Inventory tab"""
...
class StockLedger(models.Model):
    """Current stock qty per item per warehouse/rack/bin"""
...
class PurchaseInvoice(models.Model):
    ...
    mode_of_transport = models.CharField(
        max_length=20, choices=TRANSPORT_MODES, default='road', blank=True
    )
    driver = models.CharField(max_length=200, blank=True)
    transport_receipt_no = models.CharField(max_length=100, blank=True)
    transport_receipt_date = models.DateField(null=True, blank=True)
    ...
    gst_transporter_id = models.CharField(max_length=50, blank=True)
    itc_classification = models.CharField(
        max_length=100, blank=True, default='All Other ITC'
    )
...
class CounterSale(models.Model):
    ...
    type = models.CharField(max_length=10, choices=SALE_TYPES, default='sale')
...
class CounterSaleReturnItem(models.Model):
    sale_return = models.ForeignKey(
        CounterSaleReturn, on_delete=models.CASCADE, related_name='items'
    )
    item = models.ForeignKey(SparesItem, on_delete=models.PROTECT)
    quantity = models.DecimalField(max_digits=10, decimal_places=3)
    rate = models.DecimalField(max_digits=10, decimal_places=2, default=0)
...
# ── Spares Issue Alteration (Service Spares Issue Return) ─
...
class SparesIssueAlteration(models.Model):
    ...
    # job_card is a CharField here — FK to JobCard added when service module integrates
...
class SparesIssueAlterationDeleted(models.Model):
    TOOL_STATUS = [
        ('ok', 'OK'),
        ('damaged', 'Damaged'),
        ('missing', 'Missing'),
    ...
    item = models.ForeignKey(SparesItem, on_delete=models.PROTECT)
    quantity = models.DecimalField(max_digits=10, decimal_places=3)
    tool_status = models.CharField(max_length=20, choices=TOOL_STATUS)
    rack = models.ForeignKey(Rack, on_delete=models.SET_NULL, null=True, blank=True)
    bin = models.ForeignKey(Bin, on_delete=models.SET_NULL, null=True, blank=True)
    ref_no = models.CharField(max_length=100, blank=True)
 
    def __str__(self):
        return f"SIA-{self.alteration_id} | {self.item.item_code} | {self.tool_status}"
